...
modelfile = '/Users/adam/work/h2co/modeling_paper/radex_data/faure/2-2_3-3_XH2CO_fixed_faure.dat'
#stm = SmoothtauModels(modelpath+'2-2_3-3_XH2CO=1e-9_troscompt.dat')
stm = SmoothtauModels(modelfile)
...
for abund in (-10,-9,-8):
    for temperature in (20,50,80):
        for opr in (0.0, 0.1, 3.0):
            ...
            for sigma in [0.01,0.5,1.0]:
                for ii,(tbg1, tbg2) in enumerate(((2.73,2.73),)):
                    pl.figure(1)
                    pl.subplot(1,1,ii+1)
                    pl.title("$T={T}, X=10^{{{X}}}, O/P={opr}$".format(T=temperature, X=abund, opr=opr))
                    ...
                    pl.xlim(1,6)
                    pl.xlabel("log($n(H_2) [\mathrm{cm}^{-3}]$")
                    pl.ylabel("Ratio $\\tau_{obs}(2-2) / \\tau_{obs}(3-3)$")
                    pl.legend(loc='best')
            ...
            fig = pl.figure(3)
            fig.savefig("22_33_tau_ratio_vs_density_varybackground_vary_sigma_T{T}_opr{opr}_X{X}.pdf".format(T=temperature, opr=opr, X=abund))
            fig = pl.figure(2)
            fig.savefig("22_33_tau33_vs_density_varybackground_vary_sigma_T{T}_opr{opr}_X{X}.pdf".format(T=temperature, opr=opr, X=abund))
            fig = pl.figure(1)
            fig.savefig("22_33_tau22_vs_density_varybackground_vary_sigma_T{T}_opr{opr}_X{X}.pdf".format(T=temperature, opr=opr, X=abund))
